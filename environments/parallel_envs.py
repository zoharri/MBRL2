...
def make_env(env_id, seed, rank, episodes_per_task, tasks, add_done_info, **kwargs):
    def _thunk():
        ...
        if tasks is not None:
            env.unwrapped.set_possible_tasks(tasks)
        if seed is not None:
            env.seed(seed + rank)
...
def make_dream_env(seed, rank, episodes_per_task, add_done_info, vae_for_kde, decoder, encoder, base_env, use_kde,
                   kde_from_train, kde_from_running_latents, use_miuxp):
    def _thunk():
        env = gym.make('DreamEnv-v0',
                       **{'base_env': base_env, 'vae_for_kde': vae_for_kde, 'decoder': decoder, 'encoder': encoder,
                          'use_kde': use_kde, 'kde_from_train': kde_from_train,
                          'kde_from_running_latents': kde_from_running_latents, 'use_mixup': use_miuxp})
        if seed is not None:
            env.seed(seed + rank)
        if str(env.__class__.__name__).find('TimeLimit') >= 0:
            env = TimeLimitMask(env)
...
def make_vec_envs(env_name, seed, num_processes, gamma,
                  ...
                  normalise_rew, ret_rms, tasks=None, decoder=None, encoder=None, vae=None, num_dream_envs=0,
                  use_kde=False,
                  rank_offset=0,
                  add_done_info=None, parallel=False, kde_from_train=False, kde_from_running_latents=False,
                  use_mixup=None,
                  **kwargs):
    """
    ...
    envs = [make_env(env_id=env_name, seed=seed, rank=rank_offset + i,
                     ...
                     **kwargs)
            for i in range(num_processes - num_dream_envs)]
    base_env = gym.make(env_name, **kwargs)
    if tasks is not None:
        base_env.unwrapped.set_possible_tasks(tasks)
    dream_envs = [
        make_dream_env(seed, rank_offset + i, episodes_per_task, add_done_info, vae_for_kde=vae, decoder=decoder,
                       encoder=encoder,
                       base_env=base_env,
                       use_kde=use_kde, kde_from_train=kde_from_train,
                       kde_from_running_latents=kde_from_running_latents, use_miuxp=use_mixup)
        for i in range(num_dream_envs)]

    envs = envs + dream_envs

    if len(envs) > 1 and parallel is True:
