...
def visualise_dream_behaviour(args,
                              vae,
                              policy,
                              iter_idx, image_folder, kde=None):
    dream_env = gym.make('DreamEnv-v0',
                         **{'base_env': gym.make(args.env_name), 'decoder': vae.reward_decoder, 'use_kde': args.use_kde, 'kde': kde})

    dream_env = TimeLimitMask(dream_env)
    dream_env = VariBadWrapper(env=dream_env, episodes_per_task=args.max_rollouts_per_task)
    dream_env.unwrapped.visualise_behaviour(env=dream_env,
                                            args=args,
                                            policy=policy,
                                            iter_idx=iter_idx,
