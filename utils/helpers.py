...
def reset_env(env, args, indices=None, state=None, dream_env=None):
    ...
    if (indices is None) or (len(indices) == args.num_processes):
        ...
        if dream_env is not None:
            dream_state = dream_env.reset()
            # print("Current latents:")
            # for i, env in enumerate(dream_env.venv.venv.envs):
            #     print(f"latent number {i} is: {env.curr_latent}")
            state = torch.cat((state, dream_state), 0)
...
def env_step(env, action, args, dream_env=None):
    act = squash_action(action.detach(), args)
    next_obs, reward, done, infos = env.step(act[:-args.num_dream_envs]) if dream_env is not None else env.step(act)
    if dream_env is not None:
        dream_next_obs, dream_reward, dream_done, dream_infos = dream_env.step(act[-args.num_dream_envs:])

        next_obs = torch.cat((next_obs, dream_next_obs), 0)
        # next_obs[-1] = torch.from_numpy(dream_next_obs).float().to(device)
        if isinstance(reward, list):
            # print(f" un-normed dream reward: {dream_reward[0]}")
            # print(f"normalized dream reward: {dream_reward[1]}")
            reward = [torch.cat((reward[i], dream_reward[i]), 0) for i in range(len(reward))]
            # reward = [torch.cat((reward[i], dream_reward), 0) for i in range(len(reward))]
        done = np.concatenate((done, dream_done))
        # done[-1] = dream_done
        infos = infos + dream_infos
        # infos[-1] = dream_infos
    ...
    task = torch.from_numpy(np.array([curr_env.get_task() for curr_env in env.venv.venv.envs])).float().to(device)
...
#     return num_tasks
def get_num_tasks(args):
    return args.env_num_train_goals
