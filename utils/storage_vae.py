...
class RolloutStorageVAE(object):
    def __init__(self, num_processes, max_trajectory_len, zero_pad, max_num_rollouts,
                 state_dim, action_dim, vae_buffer_add_thresh, task_dim, insert_dream_data=True, num_dream_envs=0):
        """
        ...
        self.max_task_index = num_processes if insert_dream_data else num_processes - num_dream_envs
        ...
        if done.sum() == self.num_processes:  # check if we can process the entire batch at once
            ...
            if self.max_buffer_size > 0:
                if self.vae_buffer_add_thresh >= np.random.uniform(0, 1):
                    ...
                    if self.insert_idx + self.max_task_index > self.max_buffer_size:
                    ...
                    self.prev_state[:, self.insert_idx:self.insert_idx + self.max_task_index] = self.running_prev_state[
                                                                                                :,:self.max_task_index, :]
                    self.next_state[:, self.insert_idx:self.insert_idx + self.max_task_index] = self.running_next_state[
                                                                                                :,:self.max_task_index, :]
                    self.actions[:, self.insert_idx:self.insert_idx + self.max_task_index] = self.running_actions[
                                                                                             :,:self.max_task_index, :]
                    self.rewards[:, self.insert_idx:self.insert_idx + self.max_task_index] = self.running_rewards[
                                                                                             :,:self.max_task_index, :]
                    if (self.tasks is not None) and (self.running_tasks is not None):
                        insert_shape = self.tasks[self.insert_idx:self.insert_idx + self.max_task_index].shape
                        self.tasks[self.insert_idx:self.insert_idx + self.max_task_index] = self.running_tasks.reshape(
                            insert_shape)
                    self.trajectory_lens[
                    self.insert_idx:self.insert_idx + self.max_task_index] = self.curr_timestep.clone()[
                                                                             :self.max_task_index]
                    self.insert_idx += self.max_task_index
            ...
            for i in range(self.num_processes):
                ...
                if not already_reset:
                    ...
                    if done[i]:
                        ...
                        # add to permanent (up to max_buffer_len) buffer
                        if self.max_buffer_size > 0 and i < self.max_task_index:
                            if self.vae_buffer_add_thresh >= np.random.uniform(0, 1):
                                # check where to insert data
